...
splist <- parLapply(cl,flag.num,function(i){
  ...
  sp.range <- data.frame(specieKey=spls$speciesKey[i],cell=spds[,'cell'],sp.range.limit,sp.range.unlimit[,-1],sp.range.no[,-1])
...
rm(splist)
write.csv(sp.info,'output/summary/spInfo/spInfo_half.csv',row.names = F)
