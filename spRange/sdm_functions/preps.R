...
preps=function(env=parent.frame(),call){
  ...
  if(any(env$replicatetype%in%c("none","cv","block-cv","splitsample")==FALSE)){stop("Non-existing replicatetype!")}
  
  if(any(env$replicatetype=="block-cv" & is.na(env$strata))){stop("Stratum vector needed for block crossvalidation!")}
  
  if(any(env$replicatetype=="block-cv" & length(unique(env$strata))!=env$reps)){stop("Stratum vector has wrong number of levels!")}
  
  if(any(env$replicatetype%in%c("cv","block-cv","splitsample")==TRUE & (is.na("reps") ==TRUE || env$reps<2))){stop("Give reasonalbe number of replicates!")}
  
  if(any(env$replicatetype=="none" & env$reps>1)){env$reps=1;warning("Replicate type is 'none' but multiple replicates were chosen - replicates are set to 1")}
  
  
  #check for file path
  # if(any(env$save & is.na(env$project))){stop("supply project in which data should be saved!")}
  # 
  # if(any(env$save & !(env$project%in%list.dirs(env$path)))){stop(paste("Project directory not existing in",env$path,
