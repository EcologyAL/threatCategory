...
#' @param write should the projected layer be saved
#' @param tag tag to be added at the beginning of the file name
#' @return Object of class wsl.prediction with slots for meta info, and model predictions
#' @author Philipp
#' @export
#'
wsl.predict.df<-function(x,
                      predat=data.frame(),
                      thres=numeric()){
  ...
  #   oldfl=list.files(dr,pattern="Predictions",full.names = T)
  #   if(tag!=""){oldfl=grep(tag,oldfl,value=T)}
  #   if(length(oldfl)>0){
  #     unlink(oldfl)
  #   }
  #   
  ...
  for(i in 1:length(x@fits)){
    ...
    #   if(write){
    #     
    #     flnm=paste0(dr,"/Predictions_",spnm,"_",tag,
