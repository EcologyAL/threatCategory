...
red.list$climCategory <- factor(red.list$climCategory,levels=c('DD','LC','NT','VU','EN','CR','EX')) #%>% as.numeric()
...
dat <- left_join(spls[,c('speciesKey','family')],red.list[,c('speciesKey',
                                    'climCategory','intCategory_com','redlistCategory',
                                    'wt_intC','wt_intC_com','wt_redlistC')])
dat <- dat %>% left_join(sp_pa[,c('speciesKey','Protected')])

dat$category <- dat$climCategory %>% as.numeric()
dat$wt <- dat$wt_intC
...
write.csv(pdat,paths['faInfo'],row.names = F,na = '')
