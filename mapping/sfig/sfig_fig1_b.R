...
# rli_v4 <- read.csv('data_modOutput/v4/cellInfo.csv')
# rli <- rli_v4 %>% dplyr::select(cell_poll,RLI) %>% rename(RLI_v4=RLI)
# rli <- left_join(rli,rli_v4) %>% mutate(deltaRLI_v = RLI-RLI_v4)

rli_v4 <- read.csv('data_modOutput/v4/cellInfo.csv')
rli_v4 <- rli_v4 %>% dplyr::select(cell_poll,RLI,SR) %>% rename(RLI_v4=RLI)
rli_v4b <- read.csv('data_modOutput/v4b/cellInfo.csv')
rli  <- rli_v4b %>% dplyr::select(cell_poll,RLI) %>% 
  left_join(rli_v4) %>% 
  mutate(deltaRLI_v = RLI-RLI_v4)
# hist(rli$RLI_v4 - rli$RLI)
...
shp$deltaRLI_v[shp$SR<10] <- NA
shp$deltaRLI_v[!shp$land] <- NA
...
p3 <- ggplot(shp) +    
  ...
  scale_fill_gradientn(colors = mycol,
                       name=expression(RLI[FCC]),
                       na.value = 'gray95') +
  scale_colour_gradientn(colors = mycol,na.value = 'gray95',guide=F) +
...
p4 <- ggplot(shp) +    
  geom_sf(aes(fill = deltaRLI_v, colour = deltaRLI_v)) +
  scale_fill_gradient2(
    low = "brown", mid = "white", high = "purple",
    midpoint = 0,
    name = "  RLI",
    na.value = "gray95"
  ) +
...
p3
ggsave(paste0(paths['fig1_dir'],'figure1c.jpg'),plot=p3,width = 7,height = 3.5)
ggsave(paste0(paths['fig1_dir'],'figure1c.pdf'),plot=p3,width = 7,height = 3.5)

p4
ggsave(paste0(paths['fig1_dir'],'figure1d.jpg'),plot=p4,width = 7,height = 3.5)
ggsave(paste0(paths['fig1_dir'],'figure1d.pdf'),plot=p4,width = 7,height = 3.5)
