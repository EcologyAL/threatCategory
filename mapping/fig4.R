mapdat <- read.csv(paths["cellInfo"]) 

shp.data <- foreign::read.dbf(paths['baseMap'] %>% str_replace('.shp','.dbf'))
shp.data <- left_join(shp.data,mapdat)
...
ggsave(paths['fig4'],width = .8,height=.8,units='in',dpi=300)
ggsave(str_replace(paths['fig4'],'jpg','eps'),width = .8,height=.8,units='in',dpi=300)
...
for (i in 1:6) {
  ...
  paths['fig4']
  ggsave(paths['fig4'] %>% str_replace('gl',rg.pa$CONTINENT[i]),
         width = .8,height=.8,units='in',dpi=300)
  ggsave(str_replace(paths['fig4'],'jpg','eps') %>% str_replace('gl',rg.pa$CONTINENT[i]),
         width = .8,height=.8,units='in',dpi=300)
