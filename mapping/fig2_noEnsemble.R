...
#                                            redlistCategory,
#                                            category))
# red.list$ensembleCategory <- factor(red.list$ensembleCategory,levels = rev(1:7),labels = rev(c('DD','LC','NT','VU','EN','CR','EX')))

red.list.cp <- red.list[,c('ensembleCategory','redlistCategory')] %>% na.omit() %>% 
  group_by(ensembleCategory,redlistCategory) %>% mutate(n=n()) %>% distinct()
red.list.cp <- red.list.cp[order(red.list.cp$ensembleCategory,decreasing = T),]
...
{
  cairo_ps('outputs/fig2/fig2a_noEnsem.ps',width=3.9,height=3.9)
...
sp.ds.1d <- left_join(sp.ds.1d.raw,red.list[,c('speciesKey','redlistCategory','ensembleCategory')])
sp.ds.1d <- sp.ds.1d[!is.na(sp.ds.1d$ensembleCategory),]

sp.ds.1d <- sp.ds.1d %>%  
  mutate(redlistCategory=as.numeric(redlistCategory)-2,
         ensembleCategory=as.numeric(ensembleCategory)-2) %>% 
  mutate(wt_old = 1 - redlistCategory/5,
         wt_new = 1 - ensembleCategory/5) %>% 
  mutate(wt_old = ifelse(wt_old>1,NA,wt_old),
         wt_new = ifelse(wt_new>1,NA,wt_new)) 

if(max(sp.ds.1d$redlistCategory,sp.ds.1d$ensembleCategory)!=5) print(warning('MAX IS NOT 5!'))
